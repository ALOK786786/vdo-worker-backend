...
async function resolveFreshSignedUrls(url, videoFormat, audioFormat) {
  return new Promise((resolve, reject) => {
    let cmd = `yt-dlp -J --remote-components ejs:github -f "${videoFormat}+${audioFormat}" "${url}"`;
...
app.get('/api/v1/metadata', apiLimiter, async (req, res) => {
  ...
  try {
    ...
    let cmd = `yt-dlp -J --remote-components ejs:github "${url}"`;
